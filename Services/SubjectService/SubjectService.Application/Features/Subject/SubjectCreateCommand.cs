...
{
    public class SubjectCreateCommand : IRequest<ResponseRDTO<SubjectRDTO>>
    {

        public SubjectCDTO model { get; set; }

        public SubjectCreateCommand(SubjectCDTO model)
        {
            this.model = model;
        }
    }

    public class SubjectCreateCommandHandler : IRequestHandler<SubjectCreateCommand, ResponseRDTO<SubjectRDTO>>
    {
        private readonly ISubjectRepository repository;
        private readonly IMapper mapper;

        public SubjectCreateCommandHandler(ISubjectRepository repository, IMapper mapper)
        {
            this.repository = repository;
            this.mapper = mapper;
        }

        public async Task<ResponseRDTO<SubjectRDTO>> Handle(SubjectCreateCommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                SubjectModel entity = mapper.Map<SubjectModel>(request.model);
    ...
    public class SubjectCreateCommandValidator : AbstractValidator<SubjectCreateCommand>
    {
        public SubjectCreateCommandValidator()
        {
